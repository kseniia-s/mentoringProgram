...
public class PrefixNameStoryParser extends AbstractRegexParser implements StoryParser {
    ...
    public PrefixNameStoryParser(Keywords keywords) {
        this(keywords, DEFAULT_RESOURCE_LOADER, DEFAULT_TABLE_TRANSFORMERS);
    }

    public PrefixNameStoryParser(ResourceLoader resourceLoader, TableTransformers tableTransformers) {
        this(new LocalizedKeywords(), resourceLoader, tableTransformers);
    }

    public PrefixNameStoryParser(Keywords keywords, ResourceLoader resourceLoader, TableTransformers tableTransformers) {
        this(keywords, new ExamplesTableFactory(keywords, resourceLoader, tableTransformers));
    }

    public PrefixNameStoryParser(ExamplesTableFactory tableFactory) {
        this(tableFactory.keywords(), tableFactory);
    }

    public PrefixNameStoryParser(Keywords keywords, ExamplesTableFactory tableFactory) {
        super(keywords);
    ...
    public PrefixNameStoryParser(Configuration configuration) {
    ...
    public Story parseStory(String storyAsText, String storyPath) {
        ...
        if (storyPath != null) {
            story.namedAs(storyPrefix + ": " + new File(storyPath).getName());
        }
        return story;
    }
    ...
    private Lifecycle parseLifecycle(String storyAsText) {
        ...
        if (findingBeforeAndAfter.matches()) {
            ...
            List<Lifecycle.Steps> beforeSteps = parseBeforeLifecycle(beforeLifecycle);
            String afterLifecycle = findingBeforeAndAfter.group(2).trim();
            List<Lifecycle.Steps> afterSteps = parseAfterLifecycle(afterLifecycle);
        ...
        if (findingBefore.matches()) {
            ...
            List<Lifecycle.Steps> beforeSteps = parseBeforeLifecycle(beforeLifecycle);
            return new Lifecycle(beforeSteps, Arrays.<Lifecycle.Steps>asList());
        }
        Matcher findingAfter = compile(".*" + keywords().after() + "(.*)\\s*", DOTALL).matcher(lifecycle);
        if (findingAfter.matches()) {
            List<Lifecycle.Steps> beforeSteps = asList();
    ...
    private List<Lifecycle.Steps> parseBeforeLifecycle(String lifecycleAsText) {
        List<Lifecycle.Steps> list = new ArrayList<>();
        for (String byScope : lifecycleAsText.split(keywords().scope())) {
            ...
            Lifecycle.Steps steps = new Lifecycle.Steps(scope, findSteps(startingWithNL(byScope)));
            list.add(steps);
        }
        return list;
    }
    ...
    private List<Lifecycle.Steps> parseAfterLifecycle(String lifecycleAsText) {
        ...
        for (String byScope : lifecycleAsText.split(keywords().scope())) {
            ...
            for (String byOutcome : byScope.split(keywords().outcome())) {
                ...
                list.add(new Lifecycle.Steps(scope, parseOutcome(outcomeAsText), parseFilters(filtersAsText), steps));
            }
        }
        return list;
    }
